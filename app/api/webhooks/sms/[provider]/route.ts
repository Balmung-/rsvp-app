...
export async function POST(
  ...
  const rl = await rateLimit({
    key: `wh:sms:${provider}:${ipFromRequest(req)}`,
    capacity: 120,
    refillPerSecond: 10,
  });
  if (!rl.allowed) {
  ...
  for (const event of events) {
    const key = dedupeKey(["sms", provider, event.externalMessageId, event.status, event.occurredAt]);
  ...
  log.info("webhook.sms.accepted", { provider, count: events.length });
